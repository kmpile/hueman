...
~5,000 curated names from [meodai/color-names](https://github.com/meodai/color-names) bundled (the
"good names" subset — recognizable labels, not every machine-generated one).

```kotlin
val hueman = Hueman.default()

hueman.name("#ff0000")    // "Red"
hueman.name("#b56edc")    // "Lavender"
hueman.nearest("#3eb489") // NamedColor(name = "Mint", hex = "#3eb489")
```
...
```kotlin
hueman["#ff0000"]                 // operator get → "Red"
colorName("#ff0000")              // one-shot, lazy default instance
RGB("#b56edc").closestName()      // colormath Color extension
